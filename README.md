...
- 搜索页优先读取前端可见的更新时间，无法获取时再按需回退到 GitHub API
...
- 仓库页和文件树优先直接读取页面中的时间信息
- 搜索页优先读取前端时间，只有无法获取时才会询问是否改用 GitHub API
- Awesome 列表的数据来自 GitHub API；相关 API 回退提示只会记录一次，重置配置后可重新选择
